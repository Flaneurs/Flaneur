...
public class RevealLayoutDiamond extends FrameLayout {
    ...
    public RevealLayoutDiamond(Context context) {
        this(context, null);
    }

    public RevealLayoutDiamond(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public RevealLayoutDiamond(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private float getMaxRadius(int x, int y) {
        ...
        return (float) Math.sqrt(h * h + v * v) * 2;
    }
    ...
    protected boolean drawChild(@NonNull Canvas canvas, @NonNull View child, long drawingTime) {
        if (indexOfChild(child) == getChildCount() - 1) {
            ...
            path.moveTo(mClipCenterX, mClipCenterY + mClipRadius * 1.5f);
            path.lineTo(mClipCenterX + mClipRadius, mClipCenterY);
            path.lineTo(mClipCenterX, mClipCenterY - mClipRadius * 1.5f);
            path.lineTo(mClipCenterX - mClipRadius, mClipCenterY);

            path.close();

            mClipPath.addPath(path);
